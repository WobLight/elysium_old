...
bool Item::prepareLoot(Player *reciever, LootType loot_type, Player *pVictim, PermissionTypes &permission)
{
    ...
    if (!HasGeneratedLoot())
    {
        if (HasGeneratedLootSecondary()) // temporary check, merge conditions later
        {
            sLog.outError("%s attempted to regenerate %s at map %u, zone %u, area %u - grouped? %s",
                reciever->GetGuidStr().c_str(), GetGuidStr().c_str(), reciever->GetMap()->GetId(), reciever->GetZoneId(), reciever->GetAreaId(), reciever->GetGroup()? "yes" : "no");
            reciever->GetSession()->ProcessAnticheatAction("ItemsCheck", "Player::SendLoot: attempted to regenerate container loot", CHEAT_ACTION_LOG);
            return false;
        }
